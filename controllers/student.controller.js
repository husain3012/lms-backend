...
const updateDetails = async (req, res, next) => {
  ...
  if (name !== undefined) {
    const query = `update students set name = '${name}' where student_id = '${student_id}'`;
    try {
      const result = await client.query(query);
    ...
    if (oldPassword === passwordDecrypted) {
      ...
      const query = `update students set password = '${passwordEncrypted}' where student_id = '${student_id}'`;
      try {
        const result = await client.query(query);
